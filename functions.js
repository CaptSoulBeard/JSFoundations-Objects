...
function channelHasVideo(videoTitle, channel) {
  return channel.videos.some(
    (matchedVideo) => matchedVideo.title === videoTitle
  );
}
...
function getChannelByName(channelName, channels) {
  return channels.find((foundChannel) => foundChannel.name === channelName);
}
...
function searchChannels(query, channels) {
  return channels.filter(
    (filteredArray) =>
      filteredArray.name.includes(query) ||
      filteredArray.description.includes(query)
  );
}
